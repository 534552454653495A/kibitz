...
<p align="center">
  ...
  <a href="https://github.com/534552454653495A/kibitz/actions/workflows/canary-probe.yml"><img alt="canary probe" src="https://github.com/534552454653495A/kibitz/actions/workflows/canary-probe.yml/badge.svg"></a>
</p>
...
| | |
| --- | --- |
| **Works on** | `discord.com`, `canary.discord.com`, `ptb.discord.com` — server channels and DMs |
| **Needs** | Chrome 120+ · Node 22+ to build · an API key for an OpenAI-compatible endpoint or Anthropic |
...
1. Open `chrome://extensions`, turn on **Developer mode**, click **Load unpacked** and pick
   the `dist/` folder.
2. Click the Kibitz toolbar icon; it opens the settings page. Pick a provider, paste your
   key, press **Save**. Chrome asks you to allow access to **one origin**, the API you
   typed. Allow it.
...
4. Open (or reload) a Discord tab. Every message now ends with a ✦.
...
messages farthest from the one you clicked first.
...
| *Kibitz needs an API key* | Press **Configure API key** in the panel (or the toolbar icon), fill in the form, **Save**. |
...
| Claim | Evidence |
| --- | --- |
| The extension runs on real Discord Stable. | **One manual run** by the owner on 2026-09-02: buttons injected, the fiber read produced author, time and content, a scan collected 200+ messages and streamed a synthesis. One data point, not a green probe. |
...
The two DOM selectors, the React prop names, the fiber walk limits and the raw Discord
field names are verified only by the token-backed live probe. Expect breakage when
Discord ships a redesign; that is what the probe is for.
...
`MessageRecord` normalised to a `UniversalMessage`. The panel validates it, builds the
prompt from `src/core/prompts/*.md`, opens a port to the service worker, and the worker
checks settings and host permission, calls the provider and streams text back.

Five decisions carry the design. Their reasons, and the rules that follow from them, are
in [AGENTS.md §3](AGENTS.md#3-architecture-invariants).
...
4. **The key never enters the tab.** Only the service worker reads settings and makes
   HTTP calls.
5. **One file owns the Discord contract.** Every selector, prop name, message-type number
   and markup regex lives in `src/adapters/discord/selectors.ts`, with a stability
   rationale per export. `src/core/` only ever sees a platform-neutral `UniversalMessage`.
...
flowchart TD
  ...
  pr --> reprobe["canary-probe + ci on the PR"]
...
and the console errors; that is all the evidence the fix agent gets. Agents never comment,
never open issues themselves, never merge, never release.
...
| `DISCORD_PROBE_CHANNEL` | canary-probe | `<guildId>/<channelId>` of a channel that account can read, with 60+ messages including a reply and an attachment. |
...
| `npm test` | Vitest |
| `npm run check` | typecheck + tests + build; the PR gate |
...
| [CONTRIBUTING.md](CONTRIBUTING.md) | The short path through those rules for a first pull request: setup, tests, selector changes, definition of done, reporting problems. |
...
MIT — see [LICENSE](LICENSE). Not affiliated with Discord or any AI vendor. The
maintenance-pipeline *pattern* is inspired by Vencord's reporter; Vencord is GPL-3.0 and
